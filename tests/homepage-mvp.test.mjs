...
const PHASE_5F2_TERMINUS_DATE_TEXT = '2027 edition announced — exact dates not yet confirmed.';
const PHASE_5F2_TERMINUS_NOTE = 'The July 23–26, 2026 edition at Dickens / Dickens Lower has concluded. The official festival website now presents Terminus Festival 2027, but exact 2027 dates and venue were not confirmed in the August 18, 2026 check. The official shop displayed no ticket products.';
const PHASE_5F2_NA_STATUS = 'Active atlas record — 2027 dates not announced';
const PHASE_5F2_NA_SUMMARY = 'Terminus Festival remains a source-supported active atlas record for Calgary’s darkwave, EBM, industrial, synth, and post-punk scene. The July 23–26, 2026 edition has concluded. The official festival website now presents Terminus Festival 2027, but exact dates and venue have not been announced.';
const PHASE_5F2_NA_SUPPORT = 'Official and organizer-controlled sources support the concluded July 23–26, 2026 edition in Calgary, and the official festival website now presents Terminus Festival 2027.';
const PHASE_5F2_NA_SECTION = 'These are the guide cards with direct RetroAltFest atlas links. Three have official or organizer-controlled support for future 2026 dates; Terminus now has an official 2027 edition announcement, while exact dates, venue, ticketing, lineup, and logistics still need official confirmation.';
const PHASE_5F2_INDUSTRIAL_STATUS = 'Core industrial / EBM atlas record — 2027 dates not announced';
const PHASE_5F2_INDUSTRIAL_SUMMARY = 'Terminus Festival remains a strong Canadian industrial, EBM, dark electronic, darkwave, synth, and post-punk atlas link. The July 23–26, 2026 edition has concluded. The official festival website now presents Terminus Festival 2027, but exact dates and venue have not been announced.';
const PHASE_5F2_INDUSTRIAL_SUPPORT = 'Official and organizer-controlled sources support the concluded July 23–26, 2026 edition in Calgary, and the official festival website now presents Terminus Festival 2027.';
const PHASE_5F2_INDUSTRIAL_DEFINITION = 'Used for Terminus Festival, which remains an active industrial, EBM, dark electronic, and darkwave-overlap atlas link with a 2027 edition announced and exact dates and venue not yet confirmed.';
const PHASE_5F2_INDUSTRIAL_SECTION = 'Cold Waves and Terminus Festival remain the two core Industrial/EBM/dark electronic atlas links in this guide. Terminus stays linked for discovery with a 2027 edition announced, while exact dates, venue, ticketing, lineup, and logistics still need official confirmation.';
const PHASE_5F2A_MASTHEAD_CLASS = 'grid grid-cols-[repeat(auto-fit,minmax(min(100%,22rem),1fr))] items-start gap-8';
const PHASE_5F2A_PANEL_CLASS = 'relative overflow-hidden rounded-[2rem] border border-[rgba(168,85,247,0.2)] bg-[linear-gradient(145deg,rgba(35,24,57,0.82),rgba(8,7,14,0.92)_58%,rgba(3,3,6,0.96))] p-[clamp(16px,4vw,2.5rem)] shadow-[0_34px_120px_rgba(0,0,0,0.55),0_0_70px_rgba(88,28,135,0.16)]';
const PHASE_5F2A_SUMMARY_CLASS = 'mt-5 min-w-0 max-w-3xl text-lg leading-8 text-[var(--raf-text-muted)] [overflow-wrap:anywhere] [word-break:normal] sm:text-xl';
...
function normalizePhase5F2ALayout(relativePath, source) {
  ...
  return source
    ...
    .replace(PHASE_5F2A_FACT_GRID_CLASS, 'mt-8 grid gap-3 sm:grid-cols-3')
    .replace(PHASE_5F2A_FACT_CLASS, 'rounded-2xl border border-[var(--raf-border-soft)] bg-black/25 p-4')
    .replace(PHASE_5F2A_FACT_VALUE_CLASS, 'mt-2 text-sm leading-6 text-white');
}

function normalizePhase5F2Atlas(source) {
  return source.replace(/\{\n      "record_id": "raf-2026-018",[\s\S]*?\n    \}/, (block) => block
...
function normalizePhase5F2NorthAmericanGuide(source) {
  return source
    ...
    .replace(PHASE_5F2_NA_SUMMARY, 'Terminus Festival remains a source-supported active atlas record for Calgary’s darkwave, EBM, industrial, synth, and post-punk scene. The July 23–26, 2026 edition has concluded, and next edition details need official confirmation.')
    .replace(PHASE_5F2_NA_SUPPORT, 'Official and organizer-controlled sources support the concluded July 23–26, 2026 edition in Calgary.')
    .replace(PHASE_5F2_NA_SECTION, 'These are the guide cards with direct RetroAltFest atlas links. Three have official or organizer-controlled support for future 2026 dates; Terminus remains linked for discovery after its 2026 edition concluded, while next edition details need official confirmation.');
}

function normalizePhase5F2IndustrialGuide(source) {
  return source
    .replaceAll(PHASE_5F2_INDUSTRIAL_STATUS, 'Core industrial / EBM atlas record — dates not announced')
    .replace(PHASE_5F2_INDUSTRIAL_SUMMARY, 'Terminus Festival remains a strong Canadian industrial, EBM, dark electronic, darkwave, synth, and post-punk atlas link. The July 23–26, 2026 edition has concluded, and next edition details need official confirmation.')
    .replace(PHASE_5F2_INDUSTRIAL_SUPPORT, 'Official and organizer-controlled sources support the concluded July 23–26, 2026 edition in Calgary.')
    .replace(PHASE_5F2_INDUSTRIAL_DEFINITION, 'Used for Terminus Festival, which remains an active industrial, EBM, dark electronic, and darkwave-overlap atlas link while next edition details need official confirmation.')
    .replace(PHASE_5F2_INDUSTRIAL_SECTION, 'Cold Waves and Terminus Festival remain the two core Industrial/EBM/dark electronic atlas links in this guide. Terminus stays linked for discovery, but its next edition details need official confirmation.');
}
  ...
  const freshnessNormalized = relativePath === ATLAS_PATH
    ...
    : relativePath === 'src/app/guides/north-american-goth-darkwave-festivals/page.tsx'
      ...
      : relativePath === 'src/app/guides/industrial-ebm-dark-electronic-festivals-north-america/page.tsx'
        ? normalizePhase5F2IndustrialGuide(source)
        : source;
  return normalizePhase5F2ALayout(relativePath, normalizePhase5E4IAccessibility(relativePath, freshnessNormalized));
}
...
test('Terminus preserves the ended-ticket removal while applying the approved freshness correction', () => {
  ...
  for (const [relativePath, expectedHash] of Object.entries({
    ...
    'src/app/sitemap.ts': '01abde4a09f5e9dff3d2942e42c975526d8467571a2ac5a57178b663e0b76fdf',
  })) {
    assert.equal(
...
test('guides index is static, compact, and lists exactly the eight live curated guides', () => {
  ...
  const guideHrefs = [
    ...
    '/guides/planning-a-dark-alternative-festival-trip',
  ];
  ...
  assert.doesNotMatch(guidesPage, /future guide|coming soon|placeholder=|type=["']search|search form|filter|useState|useMemo|fetch\(|prisma|supabase|mongodb|\bauth\b|\bcms\b|\bdatabase\b|geocoding|latitude|longitude|coordinates|map pins/i);
});
...
test('Night Transmission Phase 4I activates New Colossus through the immutable collection and freezes route contracts', () => {
  ...
  assert.equal(createHash('sha256').update(atlasSource).digest('hex'), '261c069c3d269ceb4679e9ce98a05093c048ad5e0b3bef25b632e2e7dff22aca');
  assert.equal(createHash('sha256').update(dtoSource).digest('hex'), 'e3950b813213b93bbd700d354b45797a2cf3540637e1417c14f6e577747fccf8');
  assert.equal(createHash('sha256').update(read('src/app/sitemap.ts')).digest('hex'), '01abde4a09f5e9dff3d2942e42c975526d8467571a2ac5a57178b663e0b76fdf');
...
test('Wave-Gotik-Treffen 2027 source correction stays multi-venue, route-safe, and isolated', () => {
  ...
  assert.equal(createHash('sha256').update(css).digest('hex'), '903b3d9ad627afeec4023f543321cf6a9efbdc9663b26f419b69109a1b831dbb');
  assert.equal(createHash('sha256').update(sitemap).digest('hex'), '01abde4a09f5e9dff3d2942e42c975526d8467571a2ac5a57178b663e0b76fdf');
...
test('Phase 5B.2 publishes the selected European guide through the Guides Hub and sitemap only', () => {
  ...
  const expectedGuideOrder = [previousGuideOrder[0], europeanRoute, ...previousGuideOrder.slice(1), ticketRoute, firstTimeRoute, tripRoute];
  ...
  assert.deepEqual(guideHrefs.filter((href) => href !== europeanRoute && href !== ticketRoute && href !== firstTimeRoute && href !== tripRoute), previousGuideOrder, 'the four pre-European cards should retain exact relative order');
  assert.equal(guideHrefs.length, 8, 'the Guides Hub should expose eight guide cards after approved publication wiring');
  ...
  assert.match(guidesHub, /<em>02—08<\/em>/);
  ...
  assert.equal(staticSuffixes.length, 13, 'the sitemap should contain thirteen static routes after approved publication wiring');
  ...
  assert.equal(staticSuffixes.filter((suffix) => suffix.startsWith('/guides/')).length, 8, 'eight guide article URLs should be listed');
  assert.equal(atlas.festivals.length, 15, 'festival route count should remain 15');
  assert.equal(staticSuffixes.length + atlas.festivals.length, 28, 'the generated sitemap should contain 28 URLs');
...
test('Phase 5C.1 ticket verification guide is reader-facing, non-commercial, and locally bounded', () => {
  ...
  assert.equal((guidesHub.match(/href: "\/guides\/how-to-verify-festival-tickets-official-sources"/g) ?? []).length, 1);
...
test('Phase 5C.2 publishes the ticket verification guide through the Guides Hub and sitemap only', () => {
  const guidesHub = read('src/app/guides/page.tsx');
  const sitemap = read('src/app/sitemap.ts');
  const atlasSource = read('src/data/atlas-festivals.json');
  const atlas = JSON.parse(atlasSource);
  ...
  const expectedGuideOrder = [...previousGuideOrder, ticketRoute, firstTimeRoute, tripRoute];
  const guideHrefs = [...guidesHub.matchAll(/href: "(\/guides\/[^"]+)"/g)].map((match) => match[1]);
  const ticketCard = guidesHub.match(/\{\s*title: "How to Verify Festival Tickets and Official Sources",[\s\S]*?\n  \},/)?.[0] ?? '';

  assert.ok(ticketCard, 'the ticket-verification guide card should be extractable');
  assert.match(ticketCard, new RegExp(`title: "${ticketTitle}"`));
  assert.match(ticketCard, /href: "\/guides\/how-to-verify-festival-tickets-official-sources"/);
  assert.match(ticketCard, new RegExp(ticketCopy.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
  assert.match(ticketCard, /pre-purchase/i);
  assert.match(ticketCard, /current-edition festival dates/i);
  assert.match(ticketCard, /organizer websites/i);
  assert.match(ticketCard, /authorized ticket paths/i);
  assert.match(ticketCard, /resale listings/i);
  assert.match(ticketCard, /official sources/i);
  assert.match(ticketCard, /cannot establish that a ticket is authentic/i);
  assert.doesNotMatch(ticketCard, /guarantee|guaranteed safe tickets|scam-proof|verified tickets|certified tickets|official RetroAltFest approval|best ticket sites|trusted ticket sellers|lowest prices|buy tickets|ticket deals/i);
  ...
  assert.deepEqual(guideHrefs.filter((href) => href !== ticketRoute && href !== firstTimeRoute && href !== tripRoute), previousGuideOrder, 'all five existing cards should retain exact relative order');
  assert.equal(guideHrefs.length, 8, 'the Guides Hub should expose eight guide cards');
  ...
  assert.equal(staticSuffixes.length, 13, 'the sitemap should contain thirteen static routes');
  ...
  for (const suffix of previousStaticSuffixes) assert.ok(staticSuffixes.includes(suffix), `${suffix || '/'} should remain in the sitemap`);
  assert.deepEqual(staticSuffixes.filter((suffix) => suffix !== ticketRoute && suffix !== firstTimeRoute && suffix !== tripRoute), previousStaticSuffixes, 'existing sitemap relative order should remain exact');
  assert.equal(staticSuffixes.filter((suffix) => suffix.startsWith('/guides/')).length, 8, 'eight guide article URLs should be listed');
  assert.equal(atlas.festivals.length, 15, 'festival route count should remain 15');
  assert.equal(staticSuffixes.length + atlas.festivals.length, 28, 'the generated sitemap should contain 28 URLs');
  assert.equal((sitemap.match(/`\$\{SITE_URL\}\/guides\/how-to-verify-festival-tickets-official-sources`/g) ?? []).length, 1);
  ...
  assert.doesNotMatch(`${guidesHub}\n${sitemap}`, /date_pending|source_status|map_phase0_category|core_anchor|watchlist|Phase 0|map-readiness|needs_review|geocoding_source|geocoding_query|geocoding_confidence|latitude|longitude/i);
  assert.doesNotMatch(guidesHub, /"use client"|useState|useEffect|useMemo|fetch\(|\/api\//);
  assert.doesNotMatch(`${guidesHub}\n${sitemap}`, /[?&](?:aff|affiliate|ref|utm_|click|partner)=|Ticketmaster|StubHub|SeatGeek|Vivid Seats|AXS|Eventbrite|Viagogo/i);

  const protectedHashes = new Map([
    [ticketGuidePath, '3b7a1691ddd3b54d97a8495373fac2a5c678e33b87c7637d1663a2c8ace1137a'],
    [ticketGuideCssPath, '3ee1c6e11e59e1802d4d86fab26639a8a0c66fb7aa21a4f681c19df05fdc56b4'],
    ['src/app/verification/page.tsx', '28946389175da89f6cd91444f6693b29d21dbcfe0f4a88fa92acf8d471003f14'],
    ['src/app/verification/VerificationPage.module.css', 'b370b6781237df419a1920128d0849d24345aecab57b624826275521239f3bb1'],
    ['src/app/guides/GuidesHub.module.css', 'aef54b1c097f166f8118a0e8b2f08c07e46928f85c67f27b88fce08c344ea6e3'],
    ['src/data/atlas-festivals.json', '261c069c3d269ceb4679e9ce98a05093c048ad5e0b3bef25b632e2e7dff22aca'],
...
test('Phase 5C.3A first-time guide is practical, non-commercial, and locally bounded', () => {
  const guidePath = 'src/app/guides/first-time-dark-alternative-festival-guide/page.tsx';
  const cssPath = 'src/app/guides/first-time-dark-alternative-festival-guide/GuideArticle.module.css';

  assert.equal(existsSync(join(root, guidePath)), true, 'Phase 5C.3A route should exist locally');
  assert.equal(existsSync(join(root, cssPath)), true, 'Phase 5C.3A should use one route-local CSS module');
  ...
  assert.match(guide, /title: "First-Time Dark Alternative Festival Guide"/);
  assert.match(guide, /description:\s*"Practical preparation for a first goth, darkwave, industrial, EBM, post-punk, new-wave, or adjacent dark-alternative festival\."/);
  assert.match(guide, /path: pagePath/);
  assert.match(guide, /type: "article"/);
  assert.match(guide, /<h1[^>]*>\s*First-Time Dark Alternative Festival Guide\s*<\/h1>/s);
  assert.equal((guide.match(/<h1\b/g) ?? []).length, 1, 'the guide should have exactly one authored H1');
  assert.match(guide, /data-guide-family="night-transmission"/);
  assert.match(guide, /data-article-contract="first-time-dark-alternative-preparation-guide"/);
  assert.match(guide, /data-static-readiness-checklist/);
  assert.match(guide, /<ol[^>]+className=\{styles\.checklist\}/);
  ...
  const expectedSections = [
    ...
    'hearing-and-energy',
    'phone-power-schedule',
    'rules-before-arrival',
    'arrival-movement-return',
    'solo-or-meeting-people',
  ...
  assert.equal((guide.match(/href="\/guides\/how-to-verify-festival-tickets-official-sources"/g) ?? []).length, 1, 'the ticket guide should be linked once near the opening');
  for (const requiredRoute of ['/festivals', '/guides', '/verification']) {
    assert.match(guide, new RegExp(`href="${requiredRoute.replaceAll('/', '\\/')}"`), `${requiredRoute} should appear in the closing paths`);
  }
  for (const sceneRoute of [
    ...
    assert.doesNotMatch(guide, new RegExp(sceneRoute.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')), `${sceneRoute} should not be forced into this practical guide`);
  }

  assert.match(guide, /identified a festival/i);
  assert.match(guide, /before arrival/i);
  assert.match(guide, /indoor/i);
  assert.match(guide, /outdoor/i);
  assert.match(guide, /multi-venue/i);
  assert.match(guide, /there is no required uniform/i);
  assert.match(guide, /hearing protection/i);
  assert.match(guide, /official (?:event|festival|venue|organizer|operator)/i);
  assert.match(guide, /cannot guarantee safety, accessibility, transit, entry, or an unchanged event/i);
  assert.match(guide, /review the organizer and venue accessibility information/i);
  assert.match(guide, /recheck the official operator schedule/i);
  assert.match(guide, /consent/i);
  ...
  assert.ok(css.indexOf('tower-beacon-signature.avif') > css.indexOf('@media (min-width: 1101px)'), 'the tower URL should live only in the established desktop query');
  assert.match(css, /prefers-reduced-motion/);
  assert.match(css, /forced-colors/);
  assert.match(css, /focus-visible/);
  const nonZeroRadii = [...css.matchAll(/border-radius:\s*([^;]+);/g)]
    .map((match) => match[1].replace(/\s*!important\s*$/, '').trim())
    .filter((value) => value !== '0');
  assert.deepEqual(nonZeroRadii, [], 'Phase 5C.3A surfaces should remain square');
  ...
  assert.doesNotMatch(guide, /[?&](?:aff|affiliate|ref|utm_|click|partner)=/i);
  assert.doesNotMatch(guide, /\$\s*\d|€\s*\d|£\s*\d|\b(?:USD|EUR|GBP)\s*\d|live availability|currently on sale|sold out now|ticket deal|lowest price/i);
  assert.doesNotMatch(guide, /affiliate|commission|sponsored|coupon|hotel recommendation|flight recommendation|insurance recommendation|product recommendation|must-have product/i);
...
test('Phase 5E.1 trip-planning guide is distinct, source-safe, non-commercial, and locally bounded', () => {
  ...
  const staticRoutesBlock = sitemap.match(/const staticRoutes:[\s\S]*?\n  \];/)?.[0] ?? '';
  const staticSuffixes = [...staticRoutesBlock.matchAll(/url: (?:SITE_URL|`\$\{SITE_URL\}([^`]+)`)/g)].map((match) => match[1] ?? '');
  assert.equal(guideHrefs.length, 8, 'the Guides Hub should expose eight published guide cards after approved Phase 5E.2 wiring');
  assert.equal(guideHrefs.filter((href) => href === pagePath).length, 1, 'the Trip-Planning Guide should appear once in the Hub');
  assert.equal(staticSuffixes.length, 13, 'the sitemap should contain thirteen static routes after approved Phase 5E.2 wiring');
  assert.equal(staticSuffixes.filter((suffix) => suffix.startsWith('/guides/')).length, 8, 'the sitemap should contain eight published guide routes');
  assert.equal(staticSuffixes.filter((suffix) => suffix === pagePath).length, 1, 'the Trip-Planning canonical should appear once');
  assert.equal(atlas.festivals.length, 15, 'festival route count should remain 15');
...
test('Phase 5C.3B publishes the first-time guide and Phase 5D.3 maps only M’era Luna and NCN to it', () => {
  ...
  const expectedGuideOrder = [...previousGuideOrder, firstTimeRoute, tripRoute];
  const guideHrefs = [...guidesHub.matchAll(/href: "(\/guides\/[^"]+)"/g)].map((match) => match[1]);
  ...
  assert.deepEqual(guideHrefs.filter((href) => href !== firstTimeRoute && href !== tripRoute), previousGuideOrder, 'all six existing guide cards should retain exact relative order');
  assert.equal(guideHrefs.length, 8, 'the Guides Hub should expose eight guide cards');
  ...
  const atlasSource = read('src/data/atlas-festivals.json');
  const atlas = JSON.parse(atlasSource);
  assert.deepEqual(staticSuffixes, [...previousStaticSuffixes, firstTimeRoute, tripRoute], 'the first-time and Trip-Planning canonicals should append without reordering or removing existing URLs');
  assert.equal(staticSuffixes.length, 13, 'the sitemap should contain thirteen static routes');
  assert.equal(new Set(staticSuffixes).size, staticSuffixes.length, 'the sitemap must have zero duplicate URLs');
...
test('Phase 5E.2 publishes the trip-planning guide through the Guides Hub and sitemap only', () => {
  const checkpoint = '1d61b3b688af1470035c47f836b6b96cb1b3ae5d';
  const tripPagePath = 'src/app/guides/planning-a-dark-alternative-festival-trip/page.tsx';
  const tripCssPath = 'src/app/guides/planning-a-dark-alternative-festival-trip/GuideArticle.module.css';
  const hubPath = 'src/app/guides/page.tsx';
  const sitemapPath = 'src/app/sitemap.ts';
  const detailPath = 'src/app/festivals/[slug]/page.tsx';
  const route = '/guides/planning-a-dark-alternative-festival-trip';
  const canonical = `https://retroaltfest.com${route}`;
  const title = 'Planning a Dark Alternative Festival Trip';
  const copy = 'A practical framework for testing whether the journey around a selected festival can work, including event footprint, arrival and departure timing, accommodation search areas, late-night returns, flexibility, and final official-source checks.';
  const baseline = (path) => execFileSync('git', ['show', `${checkpoint}:${path}`], { cwd: root, encoding: 'utf8' });
  const cardBlocks = (source) => source.match(/  \{\n    title: "[^"]+",[\s\S]*?\n  \},/g) ?? [];
  const sitemapSuffixes = (source) => {
    const block = source.match(/const staticRoutes:[\s\S]*?\n  \];/)?.[0] ?? '';
    return [...block.matchAll(/url: (?:SITE_URL|`\$\{SITE_URL\}([^`]+)`)/g)].map((match) => match[1] ?? '');
  };

  const guidesHub = read(hubPath);
  const sitemap = read(sitemapPath);
  const baselineHub = baseline(hubPath);
  const baselineSitemap = baseline(sitemapPath);
  const currentCards = cardBlocks(guidesHub);
  const baselineCards = cardBlocks(baselineHub);
  const currentSuffixes = sitemapSuffixes(sitemap);
  const baselineSuffixes = sitemapSuffixes(baselineSitemap);
  const atlas = JSON.parse(read('src/data/atlas-festivals.json'));

  assert.equal(baselineCards.length, 7, 'the approved production baseline should contain seven guide cards');
  assert.equal(currentCards.length, 8, 'the Guides Hub should contain exactly eight cards');
  assert.deepEqual(currentCards.slice(0, 7), baselineCards, 'all seven existing card objects must remain byte-identical and in exact relative order');

  const tripCard = currentCards.at(-1) ?? '';
  assert.match(tripCard, new RegExp(`title: "${title}"`));
  assert.match(tripCard, /href: "\/guides\/planning-a-dark-alternative-festival-trip"/);
  assert.match(tripCard, new RegExp(copy.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
  assert.match(tripCard, /trip-feasibility planning guide/);
  assert.match(tripCard, /event footprint/);
  assert.match(tripCard, /late returns/);
  assert.match(tripCard, /official-source checks/);
  assert.equal(currentCards.filter((card) => card.includes(`href: "${route}"`)).length, 1, 'the Trip-Planning card should occur exactly once');
  assert.deepEqual(Array.from({ length: currentCards.length }, (_, index) => String(index + 1).padStart(2, '0')), ['01', '02', '03', '04', '05', '06', '07', '08']);
  assert.match(guidesHub, /<em>02—08<\/em>/);
  assert.doesNotMatch(tripCard, /\b(?:best|top|ultimate|complete|definitive|comprehensive)\b|booking recommendation|book now|buy now|hotel|airline|flight|rail provider|bus provider|rideshare|taxi|insurance|price|availability|affiliate|commission|sponsored|tracking/i);

  assert.deepEqual(currentSuffixes, [...baselineSuffixes, route], 'the new guide canonical should append after the established guide source order without moving or removing any URL');
  assert.equal(currentSuffixes.length, 13, 'the sitemap should contain thirteen static routes');
  assert.equal(new Set(currentSuffixes).size, currentSuffixes.length, 'the sitemap should contain no duplicate URL');
  assert.equal(currentSuffixes.filter((suffix) => suffix.startsWith('/guides/')).length, 8, 'the sitemap should contain eight guide article URLs');
  assert.equal(currentSuffixes.filter((suffix) => suffix === route).length, 1, 'the new canonical should occur exactly once');
  assert.equal((sitemap.match(/`\$\{SITE_URL\}\/guides\/planning-a-dark-alternative-festival-trip`/g) ?? []).length, 1);
  assert.equal(canonical, 'https://retroaltfest.com/guides/planning-a-dark-alternative-festival-trip');
  assert.equal(atlas.festivals.length, 15, 'festival-detail count should remain 15');
  assert.equal(currentSuffixes.length + atlas.festivals.length, 28, 'the generated sitemap should contain 28 unique URLs');
  ...
  assert.match(tripSource, /description:\s*"Plan a dark alternative festival trip after choosing an event: confirm the edition, interpret its footprint, choose a search area, plan late returns, and preserve flexibility\."/);
  assert.match(tripSource, /path: pagePath/);
  assert.match(tripSource, /type: "article"/);
  assert.doesNotMatch(`${guidesHub}\n${sitemap}`, /"use client"|useState|useEffect|useMemo|fetch\(|\/api\//);
  assert.doesNotMatch(`${guidesHub}\n${sitemap}`, /[?&](?:aff|affiliate|ref|utm_|click|partner)=|affiliate|commission|sponsored|hotel|airline|flight|insurance|booking platform|analytics|current price|live availability/i);
  assert.doesNotMatch(`${guidesHub}\n${sitemap}`, /date_pending|source_status|map_phase0_category|core_anchor|watchlist|Phase 0|map-readiness|needs_review|geocoding_source|geocoding_query|geocoding_confidence|latitude|longitude/i);
...
test('Phase 5E.5 aligns only M’era Luna and NCN supporting copy with the First-Time Guide', () => {
  ...
  const normalizedPage = normalizePhase5F2ALayout(pagePath, source.replaceAll(`    description: "${PHASE_5E5_FIRST_TIME_GUIDE_COPY}",\n`, ''));
...
test('Phase 5F.2 aligns Terminus 2027 freshness and only its two direct guide references', () => {
  ...
  assert.equal(atlas.metadata.record_count, 15);
  assert.equal(atlas.festivals.length, 15);
  assert.equal(atlas.festivals.filter((festival) => festival.slug === 'terminus-festival').length, 1);
  assert.equal(target.date_text, PHASE_5F2_TERMINUS_DATE_TEXT, 'Terminus still exposes the completed-2026 / no-later-edition state instead of the approved 2027 date-safe wording');
  assert.equal(target.verification_status, 'date_pending');
  assert.equal(target.data_quality_notes, PHASE_5F2_TERMINUS_NOTE);
  assert.equal(target.venue_name, null, 'the completed-2026 Dickens venue must not imply unsupported 2027 venue certainty');
  assert.equal(target.start_date, '2026-07-23');
  assert.equal(target.end_date, '2026-07-26');
  assert.doesNotMatch(`${target.date_text}\n${target.data_quality_notes}`, /2027-\d{2}-\d{2}|(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}(?:[–-]\d{1,2})?,?\s+2027/i);
  assert.doesNotMatch(target.data_quality_notes, /2027[^.]{0,80}(?:at Dickens|venue (?:is|at) Dickens)/i, 'the note must not claim Dickens as the 2027 venue');
  assert.equal(target.latitude, null);
  assert.equal(target.longitude, null);
  assert.equal(target.geocoding_source, null);
  assert.equal(target.geocoding_query, null);
  assert.equal(target.geocoding_confidence, 'not_geocoded');
  assert.deepEqual(target.source_urls, baselineTarget.source_urls);
  assert.deepEqual(target.source_links, baselineTarget.source_links);
  assert.deepEqual(target.similar_festival_ids, baselineTarget.similar_festival_ids);
  const approvedFields = new Set(['date_text', 'data_quality_notes', 'venue_name']);
  assert.deepEqual(
    Object.fromEntries(Object.entries(target).filter(([key]) => !approvedFields.has(key))),
    Object.fromEntries(Object.entries(baselineTarget).filter(([key]) => !approvedFields.has(key))),
  );
  assert.deepEqual(
    atlas.festivals.filter((festival) => festival.slug !== 'terminus-festival'),
    baselineAtlas.festivals.filter((festival) => festival.slug !== 'terminus-festival'),
    'all other 14 atlas records must remain field-identical',
  );

  for (const required of [PHASE_5F2_NA_STATUS, PHASE_5F2_NA_SUMMARY, PHASE_5F2_NA_SUPPORT, PHASE_5F2_NA_SECTION]) {
    assert.match(north, new RegExp(escapeRegex(required)));
  }
  assert.match(north, /A future edition’s dates, venue, ticketing, lineup, and logistics need official confirmation before travel planning\./);
  assert.deepEqual(normalizePhase5F2NorthAmericanGuide(north), baseline(northPath), 'only the exact approved Terminus-specific North American guide wording may change');

  assert.equal((industrial.match(new RegExp(escapeRegex(PHASE_5F2_INDUSTRIAL_STATUS), 'g')) ?? []).length, 2);
  for (const required of [PHASE_5F2_INDUSTRIAL_SUMMARY, PHASE_5F2_INDUSTRIAL_SUPPORT, PHASE_5F2_INDUSTRIAL_DEFINITION, PHASE_5F2_INDUSTRIAL_SECTION]) {
    assert.match(industrial, new RegExp(escapeRegex(required)));
  }
  assert.match(industrial, /The official site uses image-heavy assets, so RetroAltFest avoids full poster or lineup transcription unless separately verified from readable official sources\./);
  assert.match(industrial, /A future edition’s dates, venue, ticketing, lineup, and logistics need official confirmation before travel planning\./);
  assert.deepEqual(normalizePhase5F2IndustrialGuide(industrial), baseline(industrialPath), 'only the exact approved Terminus-specific Industrial guide wording may change');
  assert.doesNotMatch(`${north}\n${industrial}`, /Terminus Festival[^\n]*(?:next edition details need official confirmation|2027[^\n]*(?:exact dates|venue) (?:are|is) confirmed)/i);
  assert.doesNotMatch(`${north}\n${industrial}`, /href=["'{][^\n]*(?:terminus-festival\.com\/tickets|Eventbrite|Ticketmaster|StubHub|SeatGeek|Vivid Seats|AXS|Viagogo)|[?&](?:aff|affiliate|ref|utm_|partner)=/i);
...
test('Phase 5F.2A gives the shared legacy festival detail an intrinsic true-200% reflow contract', () => {
  ...
  assert.equal(page.includes(PHASE_5F2A_PANEL_CLASS), true, 'legacy hero padding must remain fluid without an enlarged rem floor forcing narrow content');
  assert.equal(page.includes(PHASE_5F2A_SUMMARY_CLASS), true, 'legacy hero copy must be shrinkable and emergency-wrappable without clipping');
  assert.equal(page.includes(PHASE_5F2A_FACT_GRID_CLASS), true, 'fact cards must use a text-size-aware readable minimum and intrinsic reflow');
  assert.equal(page.includes(PHASE_5F2A_FACT_CLASS), true, 'fact cards must remain shrinkable with fluid padding');
  assert.equal(page.includes(PHASE_5F2A_FACT_VALUE_CLASS), true, 'fact values must stay within their cards without truncation or hidden overflow');
